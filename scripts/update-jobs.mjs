...
async function fetchArbeitsagentur(){
  ...
  for(const wo of places){
    for(let page=1;page<=4;page++){
      const u=new URL(base);Object.entries({angebotsart:'1',wo,umkreis:'50',veroeffentlichtseit:'35',page:String(page),size:'100',pav:'false'}).forEach(([k,v])=>u.searchParams.set(k,v));
      let data;try{data=await fetchWithRetry(u,{json:true,headers:{'X-API-Key':'jobboerse-jobsuche'}});}catch(e){console.warn(`[${name}] ${wo} page ${page}: ${e.message}`);break;}
      const offers=data.stellenangebote||data.jobs||[]; if(!offers.length)break;
      for(const o of offers){
        const w=o.arbeitsort||o.arbeitsorte?.[0]||{};const c=w.koordinaten||o.koordinaten||{};
        const ref=o.refnr||o.referenznummer||o.hashId; const url=o.externeUrl||o.externeURL||(ref?`https://www.arbeitsagentur.de/jobsuche/jobdetail/${encodeURIComponent(ref)}`:'https://www.arbeitsagentur.de/jobsuche/');
        jobs.push(finalizeJob({
          id:ref?`ba-${hash(ref)}`:null,title:o.beruf||o.titel||o.stellenangebotsTitel,company:o.arbeitgeber||o.arbeitgeberName,
          location:clean([w.plz,w.ort].filter(Boolean).join(' '))||wo,address:clean([w.strasse,w.plz,w.ort].filter(Boolean).join(', ')),lat:Number(c.lat),lon:Number(c.lon),
          employmentType:o.arbeitszeitmodelle||o.arbeitszeit||[],publishedAt:o.aktuelleVeroeffentlichungsdatum||o.aktuelleVeroeffentlichungsDatum||o.veroeffentlichtAm||o.ersteVeroeffentlichungsdatum,
          url,source:name,sources:[makeSource(name,url)],remote:/heim|tele|remote|home/i.test(JSON.stringify(o.arbeitszeitmodelle||o.arbeitszeit||[]))
        }));
      }
