...
const RADIUS=Number(process.env.JOB_RADAR_RADIUS_KM||15);
...
function notEnglishOnly(j){
  const title=clean(j.title||'');const description=clean(j.description||'').slice(0,6500);const text=norm(`${title} ${description}`);if(!text)return false;
  if(GERMAN_STRONG_RE.test(`${title} ${description}`))return true;
  if(/[äöüß]/i.test(`${title} ${description}`))return true;
  const tokens=new Set(text.split(' ').filter(Boolean));let hits=0;for(const w of GERMAN_WORDS)if(tokens.has(norm(w)))hits++;
  return hits>=3;
}

async function main(){
  const payload=JSON.parse(await fs.readFile(OUT,'utf8'));const c=await center();const before=payload.jobs.length;
  ...
  payload.jobs=payload.jobs.filter(j=>{
    ...
    const p=coords(j);return Boolean(p&&km(c,p)<=RADIUS);
  });
  payload.jobs.sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0));
  payload.meta=payload.meta||{};payload.meta.generatedAt=new Date().toISOString();payload.meta.total=payload.jobs.length;
  payload.meta.scope={address:ADDRESS,radiusKm:RADIUS,pureHomeofficeGermanyOnly:true,excludeEnglishOnlyRemote:true,center:{lat:c[0],lon:c[1]}};
  payload.meta.localWithin15km=payload.jobs.filter(j=>j.remoteFull!==true).length;
  delete payload.meta.localWithin10km;
  ...
  console.log(`Scope: ${before} -> ${payload.jobs.length}; local ${payload.meta.localWithin15km}; pure remote DE/non-English ${payload.meta.remoteFull}; remote Ausland/unklar entfernt ${remoteForeignOrUnclear}; rein englisch entfernt ${remoteEnglishOnly}`);
}
