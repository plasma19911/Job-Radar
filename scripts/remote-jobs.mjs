...
const jobTypes=o=>Array.isArray(o?.job_types)?o.job_types:[o?.job_types].filter(Boolean);
...
const isHybridText=v=>/hybrid|teilweise remote|teilweise homeoffice|anteilig homeoffice|mobiles arbeiten.{0,20}(tag|tage|tagen)|homeoffice.{0,20}(tag|tage|tagen)|remote.{0,20}(tag|tage|tagen)|bis zu.{0,20}homeoffice/.test(norm(v));
const pureRemoteText=v=>{
  const t=norm(v);
  if(isHybridText(t))return false;
  return /100 prozent remote|100 remote|fully remote|full remote|komplett remote|voll remote|vollstandig remote|100 prozent homeoffice|100 homeoffice|reines homeoffice|vollstandig im homeoffice|ortsunabhangig/.test(t);
};
...
function final(o){const url=o.url;return{id:`remote-${o.slug||hash(url||o.title)}`,title:clean(o.title),company:clean(o.company_name),location:clean(o.location||'Remote'),address:'',lat:null,lon:null,remote:true,remoteFull:true,employmentType:jobTypes(o).map(clean),publishedAt:o.created_at?new Date(Number(o.created_at)*1000).toISOString():null,validThrough:null,url,source:'Arbeitnow',sources:[{name:'Arbeitnow',url}],description:strip(o.description).slice(0,6500),salary:null};}
