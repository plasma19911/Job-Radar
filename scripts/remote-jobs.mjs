...
function final(o){const url=o.url;return{id:`remote-${o.slug||hash(url||o.title)}`,title:clean(o.title),company:clean(o.company_name),location:clean(o.location||'Deutschland / Remote'),address:'',lat:null,lon:null,remote:true,remoteFull:true,employmentType:jobTypes(o).map(clean),publishedAt:o.created_at?new Date(Number(o.created_at)*1000).toISOString():null,validThrough:null,url,source:'Arbeitnow',sources:[{name:'Arbeitnow',url}],description:strip(o.description).slice(0,6500),salary:null};}
